...
智治共生团队整理的一个面向高校食堂点餐场景的菜品营养分析与健康膳食推荐工具。
...
- **单菜营养查询**：输入菜名，输出 12 项营养指标 + 占日参考量百分比 + 营养等级 + 钠警示 + 过敏原 + 膳食建议
...
`data/dishes_nutrition.json` 为**示例数据集**，来源于智治共生团队整理的学校食堂菜品（1152 道）。所有营养值依据《中国食物成分表》第 6 版，按**生重、可食部**测算，不同烹饪方式已估算添加油盐。
